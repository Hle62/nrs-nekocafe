...
const GAS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbz_KE6wMdsz4KFT3QHj1_8fDF8r3iuuvL9E-UFzi0JbXSxBdjVo7SAaNJQ6QWdGE4w1/exec'; 
// ==========================================================
...
        const productId = input.getAttribute('data-item-id'); // ★ 修正済み: data-item-idから取得
...
                const quantity = parseInt(quantityInput.value);
                
                // 商品リストから価格を取得 (quantityInputからproductIdを取得するように修正)
                const itemProductId = quantityInput.getAttribute('data-item-id');
                const product = productList.find(p => p.id === itemProductId);
                const unitPrice = product ? parseFloat(product.price) : 0; 
